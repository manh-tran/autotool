...
static void __fetch_arr_ids(NSMutableArray *arr, NSMutableArray *out, int *count)
{
    ...
    for (i = 0; i < [arr count]; ++i) {
        ...
        if ([d isKindOfClass:[NSString class]]) {
            bundle = (NSString *)d;
            if ([bundle isEqualToString:@"com.gamehd.swordsmanlegend"]) {
                *count += 1;
            } else {
                if ([bundle containsString:@"com.gamehd.swordsmanlegend"]) {
...
static NSMutableDictionary *__create_folder(const int index)
{
    ...
    dict[@"displayName"] = [NSString stringWithFormat:@"SMGroup_%d", index];
...
void wiiauto_tool_run_clone_swordman(const int argc, const char **argv)
{
    ...
    for (i = from; i <= to; ++i) {
        ...
        /* change app info */
        @try {
            NSString* path = @"/private/var/mobile/Downloads/TheCondorHeroes-mobile_template.app/Info.plist";
            NSMutableDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:path].mutableCopy; 
            
            dict[@"CFBundleName"] = [NSString stringWithFormat:@"TheCondorHeroes-mobile%d", i];
            dict[@"CFBundleDisplayName"] = [NSString stringWithFormat:@"Swordsman Rush %d", i];
            dict[@"CFBundleIdentifier"] = [NSString stringWithFormat:@"com.gamehd.swordsmanlegend%d", i];

            [dict writeToFile:@"/private/var/mobile/Downloads/TheCondorHeroes-mobile_template.app/Info.plist" atomically:YES];
            dict = nil;
        } @catch (NSException *e) {
        }

        /* copy app */
        sprintf(buf, "cp -r /private/var/mobile/Downloads/TheCondorHeroes-mobile_template.app /Applications/TheCondorHeroes-mobile%d.app", i);
        system(buf);

        // /* active app resources */
        // sprintf(buf, "/Applications/Facebook%d.app/Facebook", i);
        // system(buf);
        // system(buf);
        // system(buf);
        ...
        //     NSMutableDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:path].mutableCopy; 
        //     NSMutableArray *arr = ((NSArray *)dict[@"keychain-access-groups"]).mutableCopy;
        //     arr[0] = [NSString stringWithFormat:@"T84QZS65DQ.platformFamily%d", i];
        ...
        // sprintf(buf, "ldid -S/var/mobile/Library/WiiAuto/Entitlements/fb.xml /Applications/Facebook%d.app", i);
        // system(buf);
