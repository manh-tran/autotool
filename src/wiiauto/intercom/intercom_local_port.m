...
static void __callback(const buffer b_port, const buffer b_callback, const thread_pool pool)
{
...
roll_back:
    buffer_length(b_port, sizeof(u16), &len);    

    net_socket_new(&server);

    for (i = 0; i < len; ++i) {
        buffer_get(b_port, sizeof(u16), i, &port);
        net_socket_bind(server, port);
...
void wiiauto_intercom_register_local_port(const int num, const u16 *port, const wiiauto_intercom_callback callback)
{
    thread_job job;
    thread_pool pool;
    buffer b_port, b_callback;

    buffer_new(&b_port);
    buffer_new(&b_callback);

    buffer_append(b_port, port, sizeof(u16) * num);
    ...
    thread_job_add_arguments(job, b_port.iobj);
...
CFDataRef wiiauto_intercom_send_local_port(const int num, const u16 *port, const void *data, const u32 len)
{
    ...
    map_get(__clients__, key_obj(port[0]), &cl.iobj);
    if (!id_validate(cl.iobj)) {
        client_new(&cl);
        map_set(__clients__, key_obj(port[0]), cl.iobj);
        release(cl.iobj);
    }
    unlock(&__barrier__);
    ...
    if (!id_validate(__cl->sock.iobj)) {
        ...
        for (i = 0; i < num; ++i) {
            net_socket_connect(__cl->sock, "127.0.0.1", port[i]);
