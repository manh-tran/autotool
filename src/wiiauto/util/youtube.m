...
static void __kill_execute()
{
    ...
    FILE *fp = popen("ps -u root | grep Youtube_", "r");
    if (fp) {
...
static int __create_orig()
{
    ...
    int err = stat("/private/var/mobile/Library/WiiAuto/Clone/Youtube_orig.app", &s);
    if(-1 == err) {
        create = 1;
    } else {
        success = 1;
    ...
    if (create) {        
        NSString *url = __get_url("com.google.ios.youtube");
        if (url) {
            sprintf(buf, "cp -r %s /private/var/mobile/Library/WiiAuto/Clone/Youtube_orig.app", [url UTF8String]);
            system(buf);

            err = stat("/private/var/mobile/Library/WiiAuto/Clone/Youtube_orig.app", &s);
            if(-1 == err) {
            } else {
                success = 1;
            }
...
static int __create_template()
{
    ...
    int err = stat("/private/var/mobile/Library/WiiAuto/Clone/Youtube_template.app", &s);
    if(-1 == err) {
        create = 1;
    } else {
        success = 1;
    ...
    if (create) {
        system("mkdir /private/var/mobile/Library/WiiAuto/Clone/Youtube_template.app");
        system("ln -s /private/var/mobile/Library/WiiAuto/Clone/Youtube_orig.app/* /private/var/mobile/Library/WiiAuto/Clone/Youtube_template.app/");
        system("rm /private/var/mobile/Library/WiiAuto/Clone/Youtube_template.app/Info.plist");
        system("rm /private/var/mobile/Library/WiiAuto/Clone/Youtube_template.app/Youtube");
        system("cp /private/var/mobile/Library/WiiAuto/Clone/Youtube_orig.app/Info.plist /private/var/mobile/Library/WiiAuto/Clone/Youtube_template.app/Info.plist");
        system("cp /private/var/mobile/Library/WiiAuto/Clone/Youtube_orig.app/Youtube /private/var/mobile/Library/WiiAuto/Clone/Youtube_template.app/Youtube");
...
int wiiauto_util_clone_youtube(const char *bundle_id, const unsigned char group)
{
    ...
    @autoreleasepool {
        ...
        b1 = [b0 stringByReplacingOccurrencesOfString: @"com.google.ios.youtube" withString:@""];
...

        {
            ...
            /* change app info */
            @try {
                NSString* path = @"/private/var/mobile/Library/WiiAuto/Clone/Youtube_template.app/Info.plist";
                NSMutableDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:path].mutableCopy; 
                
                dict[@"CFBundleName"] = [NSString stringWithFormat:@"Youtube%@", b1];
                dict[@"CFBundleDisplayName"] = [NSString stringWithFormat:@"Youtube%@", b1];
                dict[@"ApplicationIdentifier"] = [NSString stringWithFormat:@"EQHXZ8M8AV.com.google.ios.youtube%@", b2];
                dict[@"CFBundleIdentifier"] = [NSString stringWithFormat:@"%@", b0];

                [dict writeToFile:@"/private/var/mobile/Library/WiiAuto/Clone/Youtube_template.app/Info.plist" atomically:YES];
                dict = nil;
            } @catch (NSException *e) {
            }

            /* copy app */
            sprintf(buf, "cp -r /private/var/mobile/Library/WiiAuto/Clone/Youtube_template.app /Applications/Youtube_%s.app", [b0 UTF8String]);
            system(buf);

            /* active app resources */
            if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"11.0")) {
                sprintf(buf, "/Applications/Youtube_%s.app/Youtube", [b0 UTF8String]);
                system(buf);
                system(buf);
                system(buf);
                system(buf);
            ...
            /* sign app */
            @try {
                NSString* path = @"/var/mobile/Library/WiiAuto/Entitlements/youtube.xml";
                NSMutableDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:path].mutableCopy; 

                {
                    NSMutableArray *arr = [NSMutableArray array];
                    [arr addObject:[NSString stringWithFormat:@"EQHXZ8M8AV.com.google.ios.youtube%@", b2]];
                    [arr addObject:[NSString stringWithFormat:@"EQHXZ8M8AV.com.google.common.SSO%@", b2]];
                    dict[@"keychain-access-groups"] = arr;
                } 
                {
                    NSMutableArray *arr = [NSMutableArray array];
                    [arr addObject:[NSString stringWithFormat:@"group.com.google.Youtube%@", b2]];
                    [arr addObject:[NSString stringWithFormat:@"group.com.google.common%@", b2]];
                    dict[@"com.apple.security.application-groups"] = arr;
                }  

                dict[@"application-identifier"] = [NSString stringWithFormat:@"EQHXZ8M8AV.com.google.ios.youtube%@", b2];        

                [dict writeToFile:@"/var/mobile/Library/WiiAuto/Entitlements/youtube.xml" atomically:YES];
                dict = nil;
            } @catch (NSException *e) {
            }

            if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"11.0")) {
                sprintf(buf, "ldid -S/var/mobile/Library/WiiAuto/Entitlements/youtube.xml /Applications/Youtube_%s.app", [b0 UTF8String]);
            } else {
                sprintf(buf, "ldid -S/var/mobile/Library/WiiAuto/Entitlements/youtube.xml /Applications/Youtube_%s.app/Youtube", [b0 UTF8String]);
            }
            system(buf);

            sprintf(buf, "/Applications/Youtube_%s.app", [b0 UTF8String]);
...
int wiiauto_util_remove_clone_youtube(const char *bundle)
{
    json_element e, e_bundle, e_tmp;
    const char *str;
    char buf[1024];
    ...
    if (str && (strlen(str) > 0) && (strstr(str, "/Applications/Youtube_"))) {
