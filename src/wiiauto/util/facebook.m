...
static void __add_app_clone(const char *bundle, const char *app_path)
{
    ...
    const char *str;
    json_element_new(&e);
    json_element_load_file(e, DAEMON_FILE_APPS_CLONED);
    ...
    json_element_save_file(e, DAEMON_FILE_APPS_CLONED);
    release(e.iobj);
}
...
static int __create_orig()
{
    ...
    int err = stat("/private/var/mobile/Library/WiiAuto/Clone/Facebook_orig.app", &s);
    if(-1 == err) {
        create = 1;
    } else {
        success = 1;
    ...
    if (create) {        
        NSString *url = __get_url("com.facebook.Facebook");
        if (url) {
            sprintf(buf, "cp -r %s /private/var/mobile/Library/WiiAuto/Clone/Facebook_orig.app", [url UTF8String]);
            system(buf);

            err = stat("/private/var/mobile/Library/WiiAuto/Clone/Facebook_orig.app", &s);
            if(-1 == err) {
            } else {
                success = 1;
            }
...
static int __create_template()
{
    ...
    int err = stat("/private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app", &s);
    if(-1 == err) {
        create = 1;
    } else {
        success = 1;
    ...
    if (create) {
        system("mkdir /private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app");
        system("ln -s /private/var/mobile/Library/WiiAuto/Clone/Facebook_orig.app/* /private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app/");
        system("rm /private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app/Info.plist");
        system("rm /private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app/Facebook");
        system("cp /private/var/mobile/Library/WiiAuto/Clone/Facebook_orig.app/Info.plist /private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app/Info.plist");
        system("cp /private/var/mobile/Library/WiiAuto/Clone/Facebook_orig.app/Facebook /private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app/Facebook");
        ...
        if ([d isKindOfClass:[NSString class]]) {
            ...
            if ([bundle isEqualToString:@"com.facebook.Facebook"]) {
                *count += 1;
                ...
                if ([bundle containsString:@"com.facebook.Facebook"]) {
...
static NSMutableDictionary *__create_folder(const int index, const char *name)
{
    ...
    dict[@"displayName"] = [NSString stringWithFormat:@"FB:%s", name];
...
static void __add_fb_ids(NSMutableDictionary *dict, NSArray *out)
{
    int i, j, sc;
...
int wiiauto_util_clone_facebook(const char *bundle_id, const unsigned char group)
{
    ...
    @autoreleasepool {
        ...
        b1 = [b0 stringByReplacingOccurrencesOfString: @"com.facebook.Facebook" withString:@""];
...

        {
            ...
            /* change app info */
            @try {
                NSString* path = @"/private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app/Info.plist";
                NSMutableDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:path].mutableCopy; 
                
                dict[@"CFBundleName"] = [NSString stringWithFormat:@"Facebook%@", b1];
                dict[@"CFBundleDisplayName"] = [NSString stringWithFormat:@"FB%@", b1];
                dict[@"ApplicationIdentifier"] = [NSString stringWithFormat:@"T84QZS65DQ.com.facebook.Facebook%@", b2];
                // dict[@"ApplicationIdentifier"] = [NSString stringWithFormat:@"T84QZS65DQ.com.facebook.Facebook", i];
                ...
                [dict writeToFile:@"/private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app/Info.plist" atomically:YES];
                dict = nil;
            } @catch (NSException *e) {
            }

            /* copy app */
            sprintf(buf, "cp -r /private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app /Applications/Facebook_%s.app", [b0 UTF8String]);
            system(buf);

            /* active app resources */
            if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"11.0")) {
                sprintf(buf, "/Applications/Facebook_%s.app/Facebook", [b0 UTF8String]);
                system(buf);
                system(buf);
                system(buf);
                system(buf);
            ...
            /* sign app */
            @try {
                NSString* path = @"/var/mobile/Library/WiiAuto/Entitlements/fb.xml";
                NSMutableDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:path].mutableCopy; 
                ...
                {
                    NSMutableArray *arr = [NSMutableArray array];
                    [arr addObject:[NSString stringWithFormat:@"group.com.facebook.Facebook%@", b2]];
                ...
                dict[@"application-identifier"] = [NSString stringWithFormat:@"T84QZS65DQ.com.facebook.Facebook%@", b2];        

                [dict writeToFile:@"/var/mobile/Library/WiiAuto/Entitlements/fb.xml" atomically:YES];
                dict = nil;
            } @catch (NSException *e) {
            }

            if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"11.0")) {
                sprintf(buf, "ldid -S/var/mobile/Library/WiiAuto/Entitlements/fb.xml /Applications/Facebook_%s.app", [b0 UTF8String]);
            } else {
                sprintf(buf, "ldid -S/var/mobile/Library/WiiAuto/Entitlements/fb.xml /Applications/Facebook_%s.app/Facebook", [b0 UTF8String]);
            }
            system(buf);

            sprintf(buf, "/Applications/Facebook_%s.app", [b0 UTF8String]);
...
int wiiauto_util_remove_clone_facebook(const char *bundle)
{
    json_element e, e_bundle, e_tmp;
    const char *str;
    char buf[1024];
    ...
    if (str && (strlen(str) > 0) && (strstr(str, "/Applications/Facebook_"))) {
...
void wiiauto_util_reinstall_facebook()
{
    ...
    common_set_status_bar("register fb clones");

    i = 0;
    json_object_iterate(e, i, &str, &e_bundle);
    while (id_validate(e_bundle.iobj)) {

        sprintf(buf, "register %s", str);
        common_set_status_bar(buf);

        json_object_require_boolean_default(e_bundle, "installed", &e_tmp, 0);
        json_boolean_get(e_tmp, &installed);
        if (installed) {

            json_object_require_string_default(e_bundle, "path", &e_tmp, "");
            json_string_get_ptr(e_tmp, &path);

            wiiauto_util_unregister_app(path);

            /* backup exec */
            sprintf(buf, "mv %s/Facebook %s/Facebook_old", path, path);
            system(buf);

            /* copy app */
            sprintf(buf, "cp /private/var/mobile/Library/WiiAuto/Clone/Facebook_template.app/Facebook %s", path);
            system(buf);

            /* active app resources */
            if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"11.0")) {
                sprintf(buf, "%s/Facebook", path);
                system(buf);
                system(buf);
                system(buf);
                system(buf);
            ...
            /* restore exec */
            sprintf(buf, "rm %s/Facebook", path);
            system(buf);

            sprintf(buf, "mv %s/Facebook_old %s/Facebook", path, path);
            system(buf);

            wiiauto_util_uicache(path);            
        }
    ...
    common_set_status_bar("register fb done");
}
        ...
        @try {
...

            {
                ...
                while (id_validate(e_bundle.iobj)) {
                    ...
                    json_boolean_get(e_tmp, &installed);
                    if (installed) {
                        NSString *b0 = [NSString stringWithUTF8String:str];
