...
static void __kill_execute()
{
    ...
    FILE *fp = popen("ps -u root | grep Firefox_", "r");
    if (fp) {
...
static void __add_app_clone(const char *bundle, const char *app_path)
{
    ...
    const char *str;
    json_element_new(&e);
    json_element_load_file(e, DAEMON_FILE_APPS_FIREFOX_CLONED);
    ...
    json_element_save_file(e, DAEMON_FILE_APPS_FIREFOX_CLONED);
    release(e.iobj);
}
...
static int __create_orig()
{
    ...
    int err = stat("/private/var/mobile/Library/WiiAuto/Clone/Firefox_orig.app", &s);
    if(-1 == err) {
        create = 1;
    } else {
        success = 1;
    ...
    if (create) {        
        NSString *url = __get_url("org.mozilla.ios.Firefox");
        if (url) {
            sprintf(buf, "cp -r %s /private/var/mobile/Library/WiiAuto/Clone/Firefox_orig.app", [url UTF8String]);
            system(buf);

            err = stat("/private/var/mobile/Library/WiiAuto/Clone/Firefox_orig.app", &s);
            if(-1 == err) {
            } else {
                success = 1;
            }
...
static int __create_template()
{
    ...
    int err = stat("/private/var/mobile/Library/WiiAuto/Clone/Firefox_template.app", &s);
    if(-1 == err) {
        create = 1;
    } else {
        success = 1;
    ...
    if (create) {
        system("mkdir /private/var/mobile/Library/WiiAuto/Clone/Firefox_template.app");
        system("ln -s /private/var/mobile/Library/WiiAuto/Clone/Firefox_orig.app/* /private/var/mobile/Library/WiiAuto/Clone/Firefox_template.app/");
        system("rm /private/var/mobile/Library/WiiAuto/Clone/Firefox_template.app/Info.plist");
        system("rm /private/var/mobile/Library/WiiAuto/Clone/Firefox_template.app/Client");
        system("cp /private/var/mobile/Library/WiiAuto/Clone/Firefox_orig.app/Info.plist /private/var/mobile/Library/WiiAuto/Clone/Firefox_template.app/Info.plist");
        system("cp /private/var/mobile/Library/WiiAuto/Clone/Firefox_orig.app/Client /private/var/mobile/Library/WiiAuto/Clone/Firefox_template.app/Client");
...
int wiiauto_util_clone_firefox(const char *bundle_id, const unsigned char group)
{
    ...
    @autoreleasepool {
        ...
        b1 = [b0 stringByReplacingOccurrencesOfString: @"org.mozilla.ios.Firefox" withString:@""];
...

        {
            ...
            /* change app info */
            @try {
                NSString* path = @"/private/var/mobile/Library/WiiAuto/Clone/Firefox_template.app/Info.plist";
                NSMutableDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:path].mutableCopy; 
                
                dict[@"CFBundleName"] = [NSString stringWithFormat:@"Firefox%@", b1];
                dict[@"CFBundleDisplayName"] = [NSString stringWithFormat:@"Firefox%@", b1];
                dict[@"CFBundleIdentifier"] = [NSString stringWithFormat:@"%@", b0];

                [dict writeToFile:@"/private/var/mobile/Library/WiiAuto/Clone/Firefox_template.app/Info.plist" atomically:YES];
                dict = nil;
            } @catch (NSException *e) {
            }

            /* copy app */
            sprintf(buf, "cp -r /private/var/mobile/Library/WiiAuto/Clone/Firefox_template.app /Applications/Firefox_%s.app", [b0 UTF8String]);
            system(buf);

            /* active app resources */
            if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"11.0")) {
                sprintf(buf, "/Applications/Firefox_%s.app/Client", [b0 UTF8String]);
                system(buf);
                system(buf);
                system(buf);
                system(buf);
            ...
            /* sign app */
            @try {
                NSString* path = @"/var/mobile/Library/WiiAuto/Entitlements/firefox.xml";
                NSMutableDictionary* dict = [NSDictionary dictionaryWithContentsOfFile:path].mutableCopy; 

                {
                    NSMutableArray *arr = [NSMutableArray array];
                    [arr addObject:[NSString stringWithFormat:@"43AQ936H96.org.mozilla.ios.Firefox%@", b2]];
                    dict[@"keychain-access-groups"] = arr;
                } 
                {
                    NSMutableArray *arr = [NSMutableArray array];
                    [arr addObject:[NSString stringWithFormat:@"group.org.mozilla.ios.Firefox%@", b2]];
                    dict[@"com.apple.security.application-groups"] = arr;
                }  

                dict[@"application-identifier"] = [NSString stringWithFormat:@"43AQ936H96.org.mozilla.ios.Firefox%@", b2];        

                [dict writeToFile:@"/var/mobile/Library/WiiAuto/Entitlements/firefox.xml" atomically:YES];
                dict = nil;
            } @catch (NSException *e) {
            }

            if (SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"11.0")) {
                sprintf(buf, "ldid -S/var/mobile/Library/WiiAuto/Entitlements/firefox.xml /Applications/Firefox_%s.app", [b0 UTF8String]);
            } else {
                sprintf(buf, "ldid -S/var/mobile/Library/WiiAuto/Entitlements/firefox.xml /Applications/Firefox_%s.app/Client", [b0 UTF8String]);
            }
            system(buf);

            sprintf(buf, "/Applications/Firefox_%s.app", [b0 UTF8String]);
...
int wiiauto_util_remove_clone_firefox(const char *bundle)
{
    json_element e, e_bundle, e_tmp;
    const char *str;
    char buf[1024];
    ...
    if (str && (strlen(str) > 0) && (strstr(str, "/Applications/Firefox_"))) {
